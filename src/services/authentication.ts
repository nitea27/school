...
export class AuthenticationService {  
    ...
    public logout(user){
        if(user){
            ...
            /* reset array of my chat on logout */
            if(this.chatService.myChats){
                ...
                if(response.status == 400) {
                  return "FAILURE";
                    ...
                    if(this.config.chat.enable_chat){
                         this.chatService.register_new_user(body.user);// make login to user or register
                    }
                    // end of registration
    ...
    public firebase_login_from_storage(user){
    // make firebase hit here to register new user
        if(this.config.chat.enable_chat){
